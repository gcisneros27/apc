...
class AuthAssignment extends CActiveRecord
{
	...
	public function  validarRoles($attribute,$params){
		
		if (!$this->hasErrors()) {
			$this->itemname;
			$modelAuth=new AuthItem();
			$lista_roles_permitidos=array();
			$lista_roles_permitidos=$modelAuth->validarRol();			
			if (!in_array($this->itemname, $lista_roles_permitidos)) {
				$this->addError('itemname','"Rol" Inválido, usted no posee permisos.');
			}
		}
	}	
